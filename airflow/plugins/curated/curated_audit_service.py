...
# Genera un run_id seguro para usar en rutas/archivos, creando uno nuevo si no existe y reemplazando caracteres invalidos
...
# Crea de forma segura toda la estructura de directorios en ADLS segmento por segmento si no existe
...
# Convierte un timestamp en milisegundos a datetime UTC y usa la hora actual si no viene valor
def _to_utc_from_ms(epoch_ms):
    if not epoch_ms:
        return datetime.now(timezone.utc)
    return datetime.fromtimestamp(epoch_ms / 1000.0, tz=timezone.utc)

# Centraliza la auditoría de ejecuciones de transformaciones curated en Airflow + Databricks, se instancia en el DAG/task de curated y se usa para registrar métricas y estado de la corrida
class CuratedRunAuditor:
    def __init__(self, *, dataset_name, layer_name, databricks_task_id):
        self.dataset_name = dataset_name
        self.layer_name = layer_name
        self.databricks_task_id = databricks_task_id

    # Construye y registra la auditoría completa de una corrida curated usando el context de Airflow, obteniendo métricas de Databricks, guardando log en ADLS y SQL, y falla la task si el estado no es SUCCESS
    def audit_from_context(self, context):
        ...
        payload = {
            "event_type": "curated_transformation",
        ...
        if payload["status"] != "SUCCESS":
            raise AirflowException(payload.get("message") or "Curated transformation failed")

    # Extrae y normaliza el run_id de Databricks desde XCom de Airflow para usarlo luego en la consulta de métricas de la corrida
    ...
    # Consulta Databricks por run_id, obtiene estado, tiempos y métricas del notebook (rows_read/rows_written) y devuelve un resumen normalizado de la corrida
    def _extract_metrics_from_databricks(self, run_id):
        ...
        if notebook_result:
            try:
                ...
                result["rows_read"] = int(metrics.get("rows_read", 0) or 0)
                result["rows_written"] = int(metrics.get("rows_written", 0) or 0)
    ...
    # Escribe el payload de auditoría como archivo JSON en ADLS bajo una ruta particionada por dataset/layer/execution_date usando credenciales de Airflow
    ...
    # Registra la auditoría de la corrida en la base SQL usando AuditLogger con métricas, estado, errores y referencias al log en ADLS
